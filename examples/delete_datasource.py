...
def main(api_client, args):
    ...
    with open("{}".format(args.data_sources_csv), 'w') as csvFile:
        ...
        for data_source_type in DATASOURCES_LIST:
            ...
            delete_datasource_fn = getattr(datasource_api, data_source_api_name["delete"])
            try:
                ...
                for data_source in  data_source_list.results:
                    datasource = delete_datasource_fn(id=data_source.entity_id)
                    logger.info("Successfully deleted {} : Response : {}".format(data_source_type, datasource))
